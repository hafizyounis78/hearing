    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        ...
        <div class="kt-subheader kt-grid__item" id="kt_subheader">
            <div class="kt-subheader__main">
                ...
                <span class="kt-subheader__desc">قائمة المرضى المحولين</span>
            </div>
...
<script>
    ...
    function viewTable() {
        ...
        table.DataTable({
            ...
            ajax: {
                "type": "post",
                "url": baseURL + 'Patient_cont/getPatientData',
                "data": {
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, full, meta) {
                        if (full.VISIT_ATTEND_CD == 108)
                            return '<span class="dropdown"><a href="#" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="dropdown" aria-expanded="true">' +
                                '<i class="la la-ellipsis-h"></i></a><div class="dropdown-menu dropdown-menu-right">' +
                                '<a class="dropdown-item" href="javascript:;" onclick="open_hearing_test_vw(' + data + ')"><i class="la la-leaf"></i> Hearing test</a>' +
                                '<a class="dropdown-item" href="javascript:;" onclick="open_referral_hearing_aid(' + data + ')"><i class="la la-archive"></i> Hearing Aid</a>' +
                                '<a class="dropdown-item" href="javascript:;" onclick="open_calendar(' + data + ')"><i class="flaticon-calendar"></i> New Appointment</a>' +
                                '<a class="dropdown-item" href="javascript:;" onclick="open_patient_file(' + data+ ',\'patient_cont\''+  ',\'referral_patient_list\''+')"><i class="flaticon-list-2"></i> Patient file</a>' +
                                '</div></span>';
                          /*  return '<a title="Risk factor" onclick="open_risk_vw(' + data + ')" class="btn btn-clean btn-icon btn-icon" href="javascript:;"><i class="la la-edit"></i></a>'
                                ...
                                + '<a title="Referral" onclick="open_calendar(' + data + ')"  class="btn  btn-clean btn-icon btn-icon" href="javascript:;" ><i class="flaticon-calendar"></i></a>'
                                + '<a title="Referral" onclick="open_patient_file(' + data + ')"  class="btn  btn-clean btn-icon btn-icon" href="javascript:;" ><i class="flaticon-list-2"></i></a>';
                        */
                           /* return '<span class="dropdown">' +
                                ...
                                '<a title="Risk factor" onclick="open_risk_vw(' + data + ')" class="btn btn-clean btn-icon btn-icon" href="javascript:;"><i class="la la-edit"></i> Risk Factor</a>' +
                                '<a title="Screening" onclick="open_screening_vw(' + data +')"  class="btn  btn-clean btn-icon btn-icon" href="javascript:;"><i class="la la-archive"></i> Screening Activity</a>' +
                                '<a title="Referral" onclick="open_referral_vw(' + data + ')"  class="btn  btn-clean btn-icon btn-icon" href="javascript:;"><i class="flaticon-reply"></i>Referral</a>' +
                                '<a title="Referral" onclick="open_calendar(' + data + ')"  class="btn  btn-clean btn-icon btn-icon" href="javascript:;"><i class="flaticon-calendar"></i> New Appointment</a>' +
                                '<a title="Referral" onclick="open_patient_file(' + data + ')"  class="btn  btn-clean btn-icon btn-icon" href="javascript:;"><i class="flaticon-list-2"></i> Patient File</a>' +
                                '</div></span>';*/
    ...
    function open_hearing_test_vw(visit_id) {
        ...
        $.ajax({
            ...
            success: function (data) {
                //   unblockUI('tbdPatientList');
                //  alert(data);
                var url = baseURL + 'screening_cont/referral_screening';
                window.location.href = url;//+national_id;
            }
        });

    }
    function open_referral_hearing_aid(visit_id) {
        ...
        $.ajax({
            ...
            success: function (data) {
                //   unblockUI('tbdPatientList');
                //  alert(data);
                var url = baseURL + 'referral_cont/referral_hearing_aid';
                window.location.href = url;//+national_id;
            }
        });
    ...
    function attend_patient(visit_id) {
        ...
        $.ajax({
            type: "POST",
            // dataType: 'json',
            url: baseURL + 'Appointment_cont/attend_visit/',
            data: {
                VISIT_ID: visit_id,
                VISIT_STATUS: 83,
            },
